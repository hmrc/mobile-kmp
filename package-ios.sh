...
if [ -z "$2" ]; then
  ...
  if [ -z "$1" ]; then
      ...
      if echo "$1" | grep -Eq "^\d{0,3}\.\d{1}\.\d{1}(-((alpha|beta)(\d{1})?))?$"; then
        ...
        plutil -remove "CFBundleSupportedPlatforms" build/xcode-frameworks/MobileKmp.framework/Info.plist
        plutil -insert "CFBundleSupportedPlatforms" -xml '<array/>' build/xcode-frameworks/MobileKmp.framework/Info.plist
        plutil -insert CFBundleSupportedPlatforms.0 -string 'iPhoneOS' build/xcode-frameworks/MobileKmp.framework/Info.plist
        echo "INFO: Copying artifact"
        cp -r build/xcode-frameworks/MobileKmp* $DIRECTORY/Build/iOS
        echo "INFO: Zipping"
        zip -r MobileKmp.framework.zip $DIRECTORY

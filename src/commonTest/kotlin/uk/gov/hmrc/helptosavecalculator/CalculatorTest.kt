...
class CalculatorTest {
    ...
    fun `Gives list of 48 months with breakdown`() {
        assertEquals(
            ...
            HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[0])
        assertEquals(25.0, HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[0].bonusToDate)

        assertEquals(
            MonthlyBreakdown(monthNumber = 2, savingsToDate = 100.0, period1Bonus = 50.0, period2Bonus = 0.0),
            HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[1])
        assertEquals(50.0, HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[1].bonusToDate)

        assertEquals(
            MonthlyBreakdown(monthNumber = 3, savingsToDate = 150.0, period1Bonus = 75.0, period2Bonus = 0.0),
            HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[2])
        assertEquals(50.0, HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[1].bonusToDate)

        assertEquals(
            MonthlyBreakdown(monthNumber = 24, savingsToDate = 1200.0, period1Bonus = 600.0, period2Bonus = 0.0),
            HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[23])
        assertEquals(600.0, HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[23].bonusToDate)

        assertEquals(
            MonthlyBreakdown(
                monthNumber = 25, savingsToDate = 1250.0, period1Bonus = 600.0, period2Bonus = 25.0),
            HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[24])
        assertEquals(625.0, HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[24].bonusToDate)

        assertEquals(
            MonthlyBreakdown(
                monthNumber = 48, savingsToDate = 2400.0, period1Bonus = 600.0, period2Bonus = 600.0),
            HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[47])
        assertEquals(1200.0, HtSCalculator.run(regularPayment = 50.0).monthlyBreakdown[47].bonusToDate)
    }

    @Test
    fun `Throw Exception when regular payment is below 1`() {
        assertFailsWith<InvalidRegularPaymentException> {
            HtSCalculator.run(regularPayment = 0.0)
        }
    }
    ...
    fun `Throw Exception when regular payment is above 50`() {
        assertFailsWith<InvalidRegularPaymentException> {
            HtSCalculator.run(regularPayment = 51.0)
        }
    }

    @Test
    fun `Month breakdown if they have saved in the first 2 months`() {
        val calculator = HtSCalculator.run(
            regularPayment = 50.0,
